...
const REDIRECT_KEY = "frontend_user_v2_redirect";
...
export function setRedirectPath(path: string | null) {
  const safe = sanitizeRedirect(path);
  if (safe) {
    session.setItem(REDIRECT_KEY, safe);
  } else {
...
export function getRedirectPath(): string | null {
  return sanitizeRedirect(session.getItem(REDIRECT_KEY));
}

export function consumeRedirectPath(): string | null {
  const value = getRedirectPath();
  session.removeItem(REDIRECT_KEY);
  return value;
}
